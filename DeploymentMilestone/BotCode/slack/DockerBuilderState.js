...
function buildDockerImage(pathToDockerFile, intermediateReply, successReply) {
    intermediateReply("Please wait... While we try to build the image", function () {
        exec("docker rmi -f tets");
        exec("docker build -t test /tmp/hello/", function (error, stdout, stderr) {
            if (error) {
                console.log(error);
                intermediateReply("Unable to create docker image, Some error exists, Please retry...");
            } else {
