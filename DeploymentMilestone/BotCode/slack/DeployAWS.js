...
function stopTask(taskArn, successReply, errorReply, intermediateReply)
{
    var params = {
        "task": taskArn
    }
    ...
    ecs.stopTask(params, function(err,data){
        ...
        {
            console.log("Stopped Old Task")
            registerTaskDefinition(successReply, errorReply, intermediateReply);
        }
    })
}

function checkTasks(taskArns, successReply, errorReply, intermediateReply)
{
    var i =0;
    var ecs = AWS.ECS()
    for(i =0 ; i< taskArns.length; i++)
    {
        var params = {
            "cluster" : "default",
            "tasks" : [taskArns[i]]
        ...
        ecs.describeTasks(params, function(err,data){
            ...
            else
            {
                ...
                console.log(data.tasks[0].taskDefinitionArn.toString())
                if(data.tasks[0].taskDefinitionArn.toString().indexOf("my-taskr1") != -1)
                    stopTask(data.tasks[0].taskArn, successReply, errorReply, intermediateReply)
            }
        })
    }
}

function stopAndRunTask(successReply, errorReply, intermediateReply) {
    var params = {
        "cluster": "default"
    }
    ...
    ecs.listTasks(params, function(err,data){
        ...
        else
        {
            ...
            checkTasks(data.taskArns, successReply, errorReply, intermediateReply)
        }
    })
}
...
// Function to stop the task if running before
function stoppingTask() {
    var params = {
        task: taskName,
        /* required */
        cluster: CONTAINER_NAME,
        reason: 'stopping the old task'
    };
    ecs.stopTask(params, function(err, data) {
        if (err) {
            console.log(err, err.stack); // an error occurred
...
// function to check whether the task is there or not
function checkTask() {
    var params = {
        familyPrefix: taskName
    };
...
function runTask(successReply, errorReply, intermediateReply) {
    ...
    if (!checkTask()) {
        errorReply("Error occured while running the task.");
    }
                ...
                exec("echo " + content + " > ~/.docker/config.json", function(er, stdo, stder) {
                    ...
                    exec(docker2, function(e, o, se) {
                        if (e) {
                            ...
                            stopAndRunTask(successReply, errorReply, intermediateReply);
